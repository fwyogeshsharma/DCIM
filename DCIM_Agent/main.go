...
import (
	...
	"github.com/faberlabs/dicm-agent/internal/agent"
...
func (p *program) Start(s service.Service) error {
	p.logger.Info("Starting DICM Agent...")
	go p.agent.Run()
	return nil
}

func (p *program) Stop(s service.Service) error {
	p.logger.Info("Stopping DICM Agent...")
	p.agent.Stop()
	return nil
}
...
func main() {
	...
	if *showVersion {
		fmt.Printf("DICM Agent v%s (built: %s)\n", version, buildTime)
		os.Exit(0)
	}
