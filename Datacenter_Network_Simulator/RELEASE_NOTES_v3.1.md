...
Version 3.1 delivers full BACnet/IP simulation for Verdigris EV2 energy monitors, live UPS power metrics, Cisco network device performance telemetry via SNMP, an expanded SNMP trap library for UPS/PDU/sensor devices, and a completely overhauled SNMP test script with per-device-type flags and auto-detection. Headless mode now initialises all simulator controllers, including BACnet and sFlow, bringing it to full parity with the desktop application.
...
These metrics are also exposed in the Live Metrics **UPS** tab as new sortable columns (Battery Voltage, Output Voltage, Output Current, Output Power, Input Current, Input Power) and appear in the Tick Panel UPS group as derived entries.

#### API

The `/api/devices` response now includes six new UPS fields:

```
ups_battery_voltage    (V)
...
### 3. Cisco Network Device SNMP Performance Metrics

Cisco switches, routers, firewalls, and load balancers now expose CPU and memory metrics via standard Cisco MIBs, live-patched on every tick.
...
### 4. SNMP Test Script — Per-Device-Type Flags
...
All output labels in `--ups` and `--pdu` sections now match the column names shown in the Live Metrics page (e.g. `Output Load`, `Input Voltage`, `Input Freq`, `Energy (kWh)`, `PDU Load`, `Real Power (W)`, `Ambient Temp`).

---

### 5. SNMP Trap Enhancements — UPS, PDU, Sensor
...
| Area | Fix |
|------|-----|
| Headless mode | `BACnetController` and `SFlowController` not registered — all BACnet API calls returned 503 in headless |
| Live Metrics — UPS | Energy (kWh) value was displaying under the Fan Status column due to a data-row misalignment |
| BACnet panel | Errors silently swallowed on start/stop — failures now surfaced as inline error messages |
...
| `GET /api/bacnet/ev2/metrics` | Live BACnet present-values for all EV2 devices |

### Extended `DeviceInfo` Response (`/api/devices`)

New fields added to UPS device payloads:
...
- Running on Linux with `--headless` now fully supports BACnet. No additional configuration required.
...
- BGP4-MIB peer entries are only populated after the first simulator tick. Querying BGP OIDs immediately after starting the simulator may return empty results.
