...
const APPLICABLE_TRAPS: Record<string, TrapEntry[]> = {
  ...
  // EVENTS only — genuine one-shots. Every UPS alarm state (on-battery, low-battery,
  // bypass, fan/battery/charger/rectifier/phase failure, input V/Hz, battery health,
  // output overload) is a CONDITION, served from the backend FAULT_MAP and shown
  // under the Conditions group — each backed by the existing ups_* state + the rule
  // engine's own raise/clear pair. Their "*_NORMAL / *_CLEARED / *_RESTORED" clears
  // are the recovery halves, not separate injectables, so they are not listed.
  ups: [
    { type: 'COLD_START',     label: 'Cold Start' },
    { type: 'AUTH_FAILURE',   label: 'Authentication Failure' },
    { type: 'TEMPERATURE_ALERT', label: 'Temperature Alert' },
  ...
  // EVENTS only — one-shot transfers/signals. The latching faults (Normal Source
  // Lost, Fail to Transfer, Not in Automatic) are CONDITIONS, served from the
  // backend FAULT_MAP and shown under the Conditions group instead.
  ats: [
