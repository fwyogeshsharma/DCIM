...
// ── PlantMetricTickModal (per-device alarm triggers) ──────────────────────────

const RUNNING_POINTS = ['Chiller_Running', 'Run_Status', 'Fan_Status', 'Unit_Running', 'Status_Modulating']

function prettyPoint(p: string): string {
  return p.replace(/^Alarm_/, '').replace(/([a-z])([A-Z])/g, '$1 $2').replace(/_/g, ' ')
}

export function PlantMetricTickModal({ deviceId, deviceName, deviceType, onClose }: { deviceId: string; deviceName: string; deviceType: string; onClose: () => void }) {
  ...
  // Resolve the device's IP once — the plant snapshot is matched on IP, which is
  // stable across renames (the BACnet device name is fixed at creation).
  useEffect(() => {
    api.device(deviceId).then(d => {
      const dev = d as DeviceInfo
      setIp(dev.ip_address || dev.mgmt_ip || null)
    }).catch(e => setErr(errorMessage(e)))
  }, [deviceId])
  ...
  async function load() {
    if (!ip) return
    try {
      const pm = await api.plantMetrics() as Array<{ ip: string; values: Record<string, number> }>
      const me = pm.find(x => x.ip === ip)
      setValues(me?.values ?? {})
      const ov = await api.plantOverrides(deviceId) as { overrides: Record<string, number> }
      setOverrides(ov.overrides ?? {})
      setErr(me ? '' : 'Device not reporting — is the BACnet plant running?')
    } catch (e) { setErr(errorMessage(e)) }
  }
  useEffect(() => {
    if (!ip) return
  ...
  async function setOv(point: string, value: number | null) {
    setBusy(point); setErr('')
    try { await api.setPlantOverride(deviceId, point, value); await load() }
    catch (e) { setErr(errorMessage(e)) }
    finally { setBusy(null) }
  }

  const alarms   = Object.keys(values).filter(k => k.startsWith('Alarm_')).sort()
  const runPoint = RUNNING_POINTS.find(k => k in values)
  const isActive = (k: string) => Number(overrides[k] ?? values[k] ?? 0) >= 0.5
  const forced   = (k: string) => k in overrides

  const Toggle = ({ on, label, busyKey, onClick }: { on: boolean; label: string; busyKey: string; onClick: () => void }) => (
    <button
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000, backdropFilter: 'blur(2px)' }}
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 6, width: 440, maxHeight: '82vh', display: 'flex', flexDirection: 'column', boxShadow: '0 16px 48px rgba(0,0,0,0.8)' }}>
        <div className="panel-header" style={{ borderRadius: '6px 6px 0 0', flexShrink: 0 }}>
          <span className="title">Metric Tick — {deviceName}</span>
          <button onClick={onClose} style={{ border: 'none', background: 'none', color: 'var(--text-muted)', fontSize: 16, cursor: 'pointer', padding: '0 4px' }}>✕</button>
        </div>

        <div style={{ padding: '12px 18px', overflowY: 'auto' }}>
          <div style={{ fontSize: 10, color: 'var(--text-dim)', marginBottom: 10 }}>
            {deviceType.replace(/_/g, ' ')} · trigger a fault on this unit only (independent of the type-wide Limits tab)
          </div>

          {runPoint && (
            <>
              <div style={SEC}>Unit</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 12 }}>
                <span style={{ flex: 1, color: 'var(--text)' }}>{prettyPoint(runPoint)}</span>
                {Number(values[runPoint] ?? 1) >= 0.5 && !forced(runPoint)
                  ? <span style={{ fontSize: 10, color: '#3fb950' }}>running</span>
                  : <span style={{ fontSize: 10, color: '#f87171' }}>stopped</span>}
                <Toggle on={forced(runPoint)} busyKey={runPoint}
                  label={forced(runPoint) ? 'Start' : 'Force Stop'}
                  onClick={() => setOv(runPoint, forced(runPoint) ? null : 0.0)} />
              </div>
            </>
          )}

          <div style={SEC}>Alarms</div>
          {alarms.length === 0 && <div style={{ color: 'var(--text-dim)', fontSize: 11 }}>No alarms for this device.</div>}
          {alarms.map(k => {
            const on = isActive(k)
            return (
              <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 12 }}>
                <span style={{ flex: 1, color: 'var(--text)' }}>
                  {prettyPoint(k)}
                  {forced(k) && <span style={{ fontSize: 9, color: '#d29922', marginLeft: 6 }}>forced</span>}
                </span>
                <span style={{ fontSize: 10, color: on ? '#f87171' : 'var(--text-dim)', width: 48, textAlign: 'right' }}>{on ? 'ACTIVE' : 'ok'}</span>
                <Toggle on={forced(k)} busyKey={k}
                  label={forced(k) ? 'Clear' : 'Trigger'}
                  onClick={() => setOv(k, forced(k) ? null : 1.0)} />
              </div>
            )
          })}
        ...
        <div style={{ padding: '8px 18px 14px', borderTop: '1px solid var(--border)', flexShrink: 0 }}>
          <button className="primary" style={{ width: '100%' }} onClick={onClose}>Close</button>
        </div>
      </div>
    </div>,
...
// ── DeviceMetricTickModal (servers / network / sensors / UPS / PDU) ───────────

type MetricSpec = { key: string; label: string; unit: string; kind: 'num' | 'state'; min?: number; max?: number; options?: string[] }

function liveValue(spec: MetricSpec, dev: DeviceInfo | null): string {
  if (!dev) return '—'
  const d = dev as unknown as Record<string, unknown>
  if (spec.key === 'memory_pct') return dev.memory_total ? Math.round(dev.memory_used / dev.memory_total * 100) + '%' : '—'
  if (spec.key === 'disk_pct')   return dev.disk_total   ? Math.round(dev.disk_used   / dev.disk_total   * 100) + '%' : '—'
  const v = d[spec.key]
  if (v == null) return '—'
  return spec.kind === 'state' ? String(v) : `${v}${spec.unit}`
}

export function DeviceMetricTickModal({ deviceId, deviceName, deviceType, onClose }: { deviceId: string; deviceName: string; deviceType: string; onClose: () => void }) {
  ...
  useEffect(() => {
    api.deviceOverridable(deviceId)
      .then(r => setSpecs((r as { metrics: MetricSpec[] }).metrics ?? []))
      .catch(e => setErr(errorMessage(e)))
  }, [deviceId])

  async function load() {
    try {
      setDev(await api.device(deviceId) as DeviceInfo)
      const ov = await api.deviceOverrides(deviceId) as { overrides: Record<string, number | string> }
      setOverrides(ov.overrides ?? {})
    } catch (e) { setErr(errorMessage(e)) }
  }
  useEffect(() => {
  ...
  async function setOv(metric: string, value: number | string | null) {
    setBusy(metric); setErr('')
    try { await api.setDeviceOverride(deviceId, metric, value); await load() }
    catch (e) { setErr(errorMessage(e)) }
    finally { setBusy(null) }
  }
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000, backdropFilter: 'blur(2px)' }}
      onMouseDown={e => e.target === e.currentTarget && onClose()}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 6, width: 480, maxHeight: '82vh', display: 'flex', flexDirection: 'column', boxShadow: '0 16px 48px rgba(0,0,0,0.8)' }}>
        <div className="panel-header" style={{ borderRadius: '6px 6px 0 0', flexShrink: 0 }}>
          <span className="title">Metric Tick — {deviceName}</span>
          <button onClick={onClose} style={{ border: 'none', background: 'none', color: 'var(--text-muted)', fontSize: 16, cursor: 'pointer', padding: '0 4px' }}>✕</button>
        </div>
        ...
        <div style={{ padding: '12px 18px', overflowY: 'auto' }}>
          <div style={{ fontSize: 10, color: 'var(--text-dim)', marginBottom: 10 }}>
            {deviceType.replace(/_/g, ' ')} · pin a live metric/state to a held value on this device only
          </div>

          {specs.length === 0 && <div style={{ color: 'var(--text-dim)', fontSize: 11 }}>No overridable metrics.</div>}
          {specs.map(m => {
            const forced = m.key in overrides
            const shown  = inputs[m.key] ?? String(forced ? overrides[m.key] : (m.kind === 'state' ? (m.options?.[0] ?? '') : ''))
            const accent = forced ? '#e3b341' : 'var(--text)'
            const fb     = forced ? '#d2992266' : 'var(--border)'
            const fbg    = forced ? '#d2992222' : 'var(--bg-hover)'
            return (
              <div key={m.key} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', fontSize: 12 }}>
                <span style={{ width: 110, color: 'var(--text)' }}>{m.label}</span>
                <span style={{ width: 70, textAlign: 'right', color: forced ? '#d29922' : 'var(--text-dim)', fontVariantNumeric: 'tabular-nums' }}>
                  {liveValue(m, dev)}
                </span>
                {m.kind === 'state' ? (
                  <select value={shown} onChange={e => setInputs(s => ({ ...s, [m.key]: e.target.value }))}
                    style={{ width: 96, padding: '2px 4px', fontSize: 11, background: 'var(--bg-hover)', border: '1px solid var(--border)', borderRadius: 4, color: 'var(--text)' }}>
                    {(m.options ?? []).map(o => <option key={o} value={o}>{o}</option>)}
                  </select>
                ) : (
                  <input type="number" min={m.min} max={m.max} value={shown}
                    onChange={e => setInputs(s => ({ ...s, [m.key]: e.target.value }))}
                    style={{ width: 70, padding: '2px 6px', fontSize: 11, background: 'var(--bg-hover)', border: '1px solid var(--border)', borderRadius: 4, color: 'var(--text)' }} />
                )}
                <button disabled={busy === m.key} onClick={() => setOv(m.key, m.kind === 'state' ? shown : parseFloat(shown))}
                  style={{ minWidth: 56, padding: '3px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', background: fbg, border: `1px solid ${fb}`, color: accent }}>
                  {busy === m.key ? '…' : forced ? 'Update' : 'Force'}
                </button>
                <button disabled={busy === m.key || !forced} onClick={() => { setInputs(s => { const n = { ...s }; delete n[m.key]; return n }); setOv(m.key, null) }}
                  style={{ minWidth: 50, padding: '3px 8px', borderRadius: 4, fontSize: 11, cursor: forced ? 'pointer' : 'not-allowed',
                    background: 'transparent', border: '1px solid var(--border)', color: forced ? 'var(--text)' : 'var(--text-dim)' }}>
                  Clear
                </button>
              </div>
            )
...
const PLANT_TYPES = ['chiller', 'pump', 'cooling_tower', 'valve', 'cdu', 'crah']
const METRIC_TICK_TYPES = [...PLANT_TYPES, 'server', 'router', 'switch', 'firewall', 'load_balancer', 'oob_switch', 'ups', 'pdu', 'floor_pdu', 'sensor']

export default function NodeContextMenu({ nodeId, deviceType, deviceName, modelName, x, y, onClose, onLocate, onEditDevice, onShowInfo, onMetricTick }: Props) {
  ...
  const menu = (
    ...
    >
      ...
      {METRIC_TICK_TYPES.includes(deviceType) && (
        <MenuItem label="Metric Tick…"  onClick={() => { onClose(); onMetricTick() }} />
      )}
