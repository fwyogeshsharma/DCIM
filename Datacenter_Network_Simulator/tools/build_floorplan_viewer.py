...
// Colour along a streamline parameter s in [0,1]: cyan cold supply, warming to
// orange-red as it passes THROUGH the rack (s 0.5..0.75), then hot. Alpha fades
// in/out at the loop ends so the recycle from ceiling back to floor is invisible.
function _flowColAt(col,i,s){
  const fade = s<0.07 ? s/0.07 : s>0.93 ? (1-s)/0.07 : 1;
  const m = s<0.5 ? 0 : s>0.75 ? 1 : (s-0.5)/0.25;          // cold->hot transition through the rack
  const r=0.18+(0.88-0.18)*m, gc=0.62-(0.62-0.30)*m, b=0.95-(0.95-0.18)*m;
  col[i*3]=r*fade; col[i*3+1]=gc*fade; col[i*3+2]=b*fade;
}
// Build a streamline per IT rack tracing the real raised-floor airflow loop:
//   P0 cold-aisle floor -> P1 rise in cold aisle -> P2 INTO the rack front ->
//   P3 OUT the rack rear into the hot aisle -> P4 rise to the ceiling return.
// Particles ride these paths (stepFlow), so the smoke flows in and out of the
// cabinets instead of drifting straight up.
function buildFlow(root,g,racks,cx,cz){
  ...
  const yFloor=(three.flr||RF)+0.04, yMid=(three.flr||RF)+RACK_H*0.5, yTop=(three.flr||RF)+2.6;
  const paths=[];
  for(const r of racks){ if(!(r.device_count>0)) continue;     // skip empty cells / facility gear
    ...
    paths.push(new Float32Array([ rx,yFloor,cZ,  rx,yMid,cZ,  rx,yMid,rz,  rx,yMid,hZ,  rx,yTop,hZ ]));
  }
  if(!paths.length){ three.flow=null; return; }
  const PER=12, NP=paths.length, N=NP*PER, key='flow#'+NP;
  let s,spd,pi;
  if(three._flowKey===key && three._flowS && three._flowS.length===N){
    s=three._flowS; spd=three._flowSpd; pi=three._flowPi;       // continue (no jump on rebuild)
  } else {
    s=new Float32Array(N); spd=new Float32Array(N); pi=new Int16Array(N);
    for(let i=0;i<N;i++){ pi[i]=(i/PER)|0; s[i]=Math.random(); spd[i]=0.0022+Math.random()*0.0016; }
    three._flowKey=key; three._flowS=s; three._flowSpd=spd; three._flowPi=pi;
  }
  const pos=new Float32Array(N*3), col=new Float32Array(N*3);
  const geo=new THREE.BufferGeometry();
  geo.setAttribute('position',new THREE.BufferAttribute(pos,3));
  geo.setAttribute('color',new THREE.BufferAttribute(col,3));
  const mat=new THREE.PointsMaterial({size:0.3, map:softDotTex(), vertexColors:true,
    transparent:true, opacity:0.7, depthWrite:false, sizeAttenuation:true,
    blending:THREE.AdditiveBlending});
  const pts=new THREE.Points(geo,mat); root.add(pts);
  three.flow={pts,pos,col,s,spd,pi,paths};
  stepFlow(true);   // place particles before the first render
}
function stepFlow(noAdvance){ const f=three.flow; if(!f) return;
  const {pos,col,s,spd,pi,paths}=f, n=s.length;
  for(let i=0;i<n;i++){
    if(!noAdvance){ s[i]+=spd[i]; if(s[i]>=1) s[i]-=1; }
    const p=paths[pi[i]], u=s[i]*4, seg=Math.min(3,u|0), t=u-seg, a=seg*3, b=a+3;
    pos[i*3]  =p[a]  +(p[b]  -p[a]  )*t;
    pos[i*3+1]=p[a+1]+(p[b+1]-p[a+1])*t;
    pos[i*3+2]=p[a+2]+(p[b+2]-p[a+2])*t;
    _flowColAt(col,i,s[i]);
  }
