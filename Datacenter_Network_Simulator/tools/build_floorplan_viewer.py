...
function makeRack(r,ds,m){ const grp=new THREE.Group(); const W=FOOT.width,D=FOOT.depth;
  ...
  if(S.heat && heatReady(m)){ const v=m.val(r); if(v>0){ const c=bandColor(v,m.bands);
    ...
    // NB: no full-height translucent glow box. Several large additive boxes with
    // depthWrite off overlap each other + the floor plane, so the transparent
    // sort order flips every frame (OrbitControls damping keeps the camera micro-
    // moving) -> visible blinking. Instead use only blink-free primitives:
    //  (a) an OPAQUE cap on top (depthWrite on -> never re-sorted), taller for
    //      hotter racks so it still reads from iso/top;
    const ch=0.05+0.12*t;
    const cap=new THREE.Mesh(new THREE.BoxGeometry(W+0.09,ch,D+0.09),
      new THREE.MeshBasicMaterial({color:col})); cap.position.y=RACK_H+0.04+ch/2; grp.add(cap);
    //  (b) one flat colour panel on the rack FRONT (inlet face). A single depth-
    //      tested quad per rack -> neighbours occlude it correctly, nothing
    //      overlaps itself, so no z-fight / flicker. Bolder the hotter the rack.
    const fp=new THREE.Mesh(new THREE.PlaneGeometry(W*0.9,RACK_H*0.96),
      new THREE.MeshBasicMaterial({color:col,transparent:true,opacity:0.3+0.4*t,depthWrite:false}));
    fp.position.set(0,RACK_H/2,D/2+0.035); grp.add(fp); } }
  return grp; }
