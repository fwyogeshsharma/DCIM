...
def plan_breakers(graph: dict, pdu_phases: dict, panel_names: set,
    ...
    """Assign each rack PDU a breaker position on its RPP: next free, both columns.

    Panelboard poles run 1,3,5... down the left column and 2,4,6... down the right.
    This used to read "A feeds odd, B feeds even" so each redundant side sat in its
    own column — but that describes a board fed from both sides, and no RPP in this
    estate is: RPPA feeds eight PDUAs and no PDUBs. The rule left the entire even
    column of every panel empty, which is not a schedule anyone could have read off
    real gear. Positions are handed out lowest-free-first across both columns
    instead, and the A/B split is already visible in the PANEL a PDU lands on.
...
def breaker_sql(rows: list[dict]) -> str:
    ...
    out = [
        ...
        "-- Generated by tools/export_to_opendcim.py. Positions are handed out",
        "-- lowest-free-first across both columns; a 3-pole breaker takes three",
        "-- same-parity poles. The A/B split shows in WHICH PANEL a PDU lands on,",
        "-- not in the column: every RPP here feeds one side only.",
