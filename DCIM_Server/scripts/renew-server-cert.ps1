...
$configFile = "$CertsDir\server_renewal.cnf"
...
openssl req -new -key $ServerKey -out "$CertsDir\server.csr" -config $configFile 2>&1 | Out-Null
Write-Host "   [OK] CSR generated" -ForegroundColor Green

# Sign with CA
Write-Host "3. Signing certificate with CA..." -ForegroundColor Cyan
openssl x509 -req -in "$CertsDir\server.csr" `
    -CA $CACert `
    -CAkey $CAKey `
    -CAcreateserial `
    -out $ServerCert `
...
Remove-Item "$CertsDir\server.csr" -Force -ErrorAction SilentlyContinue
