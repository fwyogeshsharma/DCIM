...
if ($caCert) {
    Show-CertificateStatus "CA" $caCert ".\generate-certs.ps1"
}

# Check server certificate
$serverCert = Get-CertificateExpiry "..\..\certs\server.crt"
if ($serverCert) {
    Show-CertificateStatus "Server" $serverCert ".\renew-server-cert.ps1"
}

# Check client certificate
$clientCert = Get-CertificateExpiry "..\..\certs\client.crt"
if ($clientCert) {
    Show-CertificateStatus "Client" $clientCert ".\renew-client-cert.ps1 -AgentName <agent-name>"
}
...
if (Test-Path "..\..\certs\agents") {
    ...
    if ($agentDirs.Count -gt 0) {
        ...
        foreach ($agentDir in $agentDirs) {
            ...
            if ($agentCert) {
                Show-CertificateStatus "  $($agentDir.Name)" $agentCert ".\renew-client-cert.ps1 -AgentName $($agentDir.Name)"
