...
if ([string]::IsNullOrWhiteSpace($ServerName)) {
    Write-Host "Error: Server name cannot be empty" -ForegroundColor Red
    exit 1
}
...
# Get script directory and set paths relative to DCIM_Server root
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$scriptsDir = Split-Path -Parent $scriptDir
$rootDir = Split-Path -Parent $scriptsDir
$certsDir = Join-Path $rootDir "certs"
$serverDir = Join-Path $certsDir "servers\$ServerName"

# Check if CA exists in root certs directory
if (-not (Test-Path "$certsDir\ca.crt") -or -not (Test-Path "$certsDir\ca.key")) {
    Write-Host "Error: CA certificate not found!" -ForegroundColor Red
    Write-Host "Make sure $certsDir\ca.crt and $certsDir\ca.key exist" -ForegroundColor Yellow
    Write-Host "Run .\scripts\windows\generate-certs.ps1 first to create CA" -ForegroundColor Yellow
    exit 1
}
...
# Create agent directory under server
$agentDir = "$serverDir\agents\$AgentName"
if (-not (Test-Path $agentDir)) {
    New-Item -ItemType Directory -Path $agentDir -Force | Out-Null
}

Write-Host "Generating client certificate for agent: $AgentName" -ForegroundColor Yellow
...
# Copy CA certificate
Copy-Item "$certsDir\ca.crt" "$agentDir\ca.crt" -Force

# Clean up temporary files
Remove-Item "$agentDir\client.csr" -Force -ErrorAction SilentlyContinue
Remove-Item "$agentDir\client.cnf" -Force -ErrorAction SilentlyContinue
