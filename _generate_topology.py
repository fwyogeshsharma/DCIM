...
def build_three_tier_datacenter():
    ...
    core_r1 = t.add("Core-R1", DeviceType.ROUTER, Vendor.CISCO,    8, 600, CORE_Y)
    core_r2 = t.add("Core-R2", DeviceType.ROUTER, Vendor.JUNIPER,  8, 900, CORE_Y)
    ...
    for pod_idx, (pod_label, pod_cx) in enumerate([("A", 450), ("B", 1050)]):
        ...
        for ai, ax in enumerate(agg_xs):
            agg = t.add(f"Agg-SW{pod_idx*2+ai+1}",
                        DeviceType.SWITCH, Vendor.CISCO, 48, ax, AGG_Y)
            aggs.append(agg)
            t.link(core_r1, agg)
            t.link(core_r2, agg)
        ...
        for ti, tx in enumerate(tor_xs):
            tor = t.add(f"ToR-{pod_label}{ti+1}",
                        DeviceType.SWITCH, Vendor.CISCO, 48, tx, TOR_Y)
            all_tors.append(tor)
            ...
            for si, sx in enumerate(srv_xs):
                ...
                vendor = Vendor.LINUX if si % 3 != 2 else Vendor.GENERIC
...
def build_spine_leaf():
    ...
    # ---- Border router ----
    border = t.add("Border-GW", DeviceType.ROUTER, Vendor.JUNIPER, 8, 600, -80)

    # ---- OOB management switch ----
    oob = t.add("OOB-MGMT-SW", DeviceType.SWITCH, Vendor.CISCO, 48, 1250, 100)
    t.link(border, oob)

    # ---- Spine layer ----
    spines = []
    for i, sx in enumerate(SPINE_XS):
        sp = t.add(f"Spine-{i+1}", DeviceType.SWITCH, Vendor.CISCO, 32, sx, SPINE_Y)
        spines.append(sp)
        t.link(border, sp)
    ...
    for li, lx in enumerate(LEAF_XS):
        leaf = t.add(f"Leaf-{li+1:02d}", DeviceType.SWITCH, Vendor.CISCO, 48, lx, LEAF_Y)
        leaves.append(leaf)
        t.link(oob, leaf)
    ...
    for li, leaf in enumerate(leaves):
        ...
        for si in range(srv_count):
            ...
            vendor = random.choice([Vendor.LINUX, Vendor.LINUX, Vendor.GENERIC])
    ...
    for si, leaf in enumerate(leaves[:2]):
        base_x = LEAF_XS[si]
        stor = t.add(f"Storage-{si+1}", DeviceType.SERVER, Vendor.LINUX,
...
def build_enterprise_wan():
    ...
    # ---- Internet / WAN cloud ----
    isp1 = t.add("ISP1-Router",   DeviceType.ROUTER, Vendor.JUNIPER, 8,  200, 60)
    isp2 = t.add("ISP2-Router",   DeviceType.ROUTER, Vendor.CISCO,   8,  500, 60)
    fw   = t.add("Firewall-GW",   DeviceType.ROUTER, Vendor.GENERIC, 4,  350, 180)
    t.link(isp1, fw)
    t.link(isp2, fw)

    # ---- HQ Site (x-offset 0, y-offset 300) ----
    hq_wan  = t.add("HQ-WAN-R",     DeviceType.ROUTER, Vendor.CISCO, 8,  350, 340)
    hq_core = t.add("HQ-Core-SW",   DeviceType.SWITCH, Vendor.CISCO, 48, 350, 500)
    t.link(fw, hq_wan)
    ...
    for i, ax in enumerate(hq_access_xs):
        ac = t.add(f"HQ-Access-{i+1}", DeviceType.SWITCH, Vendor.CISCO, 24, ax, 660)
    ...
    for ai, ac in enumerate(hq_accesses):
        ...
        for di in range(8):
            ...
            srv = t.add(f"HQ-PC-{ai*8+di+1:02d}",
                        DeviceType.SERVER, Vendor.LINUX, 1, dx, 820)
            t.link(ac, srv)

    # HQ servers: 2 dedicated rack switches, 10 servers each
    for rack in range(2):
        srv_sw = t.add(f"HQ-Rack-SW{rack+1}", DeviceType.SWITCH, Vendor.CISCO,
                       24, 680 + rack * 180, 500)
        t.link(hq_core, srv_sw)
        for si in range(10):
            srv = t.add(f"HQ-SRV-{rack*10+si+1:02d}",
                        DeviceType.SERVER, Vendor.LINUX, 2,
                        580 + rack * 180 + (si - 4.5) * 65, 660)
            t.link(srv_sw, srv)

    # ---- Site A — dist switch + 3 access switches + 10 PCs each ----
    site_a_wan = t.add("SiteA-WAN-R", DeviceType.ROUTER, Vendor.CISCO, 4, 1150, 340)
    t.link(fw, site_a_wan)
    siteA_dist = t.add("SiteA-Dist-SW", DeviceType.SWITCH, Vendor.CISCO, 24, 1150, 480)
    t.link(site_a_wan, siteA_dist)
    for i, ax in enumerate([1000, 1150, 1300]):
        ac = t.add(f"SiteA-Access-{i+1}", DeviceType.SWITCH, Vendor.CISCO, 24, ax, 620)
        t.link(siteA_dist, ac)
        for di in range(10):
            dx = ax + (di - 4.5) * 50
            pc = t.add(f"SiteA-PC-{i*10+di+1:02d}",
                       DeviceType.SERVER, Vendor.LINUX, 1, dx, 780)
            t.link(ac, pc)

    # ---- Site B — dist switch + 3 access switches + 10 PCs each ----
    site_b_wan = t.add("SiteB-WAN-R", DeviceType.ROUTER, Vendor.JUNIPER, 4, 1700, 340)
    t.link(fw, site_b_wan)
    siteB_dist = t.add("SiteB-Dist-SW", DeviceType.SWITCH, Vendor.CISCO, 24, 1700, 480)
    t.link(site_b_wan, siteB_dist)
    for i, ax in enumerate([1550, 1700, 1850]):
        ac = t.add(f"SiteB-Access-{i+1}", DeviceType.SWITCH, Vendor.CISCO, 24, ax, 620)
...
def build_hyperscale_pod():
    ...
    # ---- Cluster routers ----
    cr1 = t.add("Cluster-R1", DeviceType.ROUTER, Vendor.CISCO,   8,  500, CLUSTER_Y)
    cr2 = t.add("Cluster-R2", DeviceType.ROUTER, Vendor.CISCO,   8,  900, CLUSTER_Y)
    t.link(cr1, cr2)

    # Load balancers at the top
    lb1 = t.add("LB-1", DeviceType.SERVER, Vendor.GENERIC, 4, 300, CLUSTER_Y)
    lb2 = t.add("LB-2", DeviceType.SERVER, Vendor.GENERIC, 4, 1100, CLUSTER_Y)
    t.link(cr1, lb1)
    t.link(cr2, lb2)

    # ---- Fabric switches (2) ----
    fab1 = t.add("Fabric-SW1", DeviceType.SWITCH, Vendor.CISCO, 64, 550, FABRIC_Y)
    fab2 = t.add("Fabric-SW2", DeviceType.SWITCH, Vendor.CISCO, 64, 850, FABRIC_Y)
    ...
    for i, px in enumerate(pod_xs):
        pod = t.add(f"Pod-SW{i+1}", DeviceType.SWITCH, Vendor.CISCO, 48, px, POD_Y)
        pods.append(pod)
        t.link(fab1, pod)
        t.link(fab2, pod)
    ...
    for pi, pod in enumerate(pods):
        ...
        for ti in range(2):
            ...
            tor = t.add(f"ToR-P{pi+1}-{ti+1}", DeviceType.SWITCH, Vendor.CISCO, 48,
                        tx, TOR_Y)
            tors.append(tor)
            t.link(pod, tor)
    ...
    for ti, tor in enumerate(tors):
        ...
        for si in range(srv_per_tor):
            ...
            srv = t.add(f"Compute-T{ti+1}-{si+1:02d}",
                        DeviceType.SERVER, Vendor.LINUX, 2, sx, SRV_Y)
            t.link(tor, srv)
    ...
    for gi, tor in enumerate(tors[:4]):
        ...
        gpu = t.add(f"GPU-Server-{gi+1}", DeviceType.SERVER, Vendor.LINUX, 4,
                    cx + 200, SRV_Y + 120)
        t.link(tor, gpu)
    ...
    for si, tor in enumerate(tors[4:]):
        ...
        sto = t.add(f"Storage-{si+1}", DeviceType.SERVER, Vendor.LINUX, 4,
                    cx - 200, SRV_Y + 120)
        t.link(tor, sto)
